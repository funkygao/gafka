...
func (*Status) Synopsis() string {
	return "Diagnose online high level consumer group status on a topic"
}
...
Usage: %s status [options]
    ...
    Diagnose online high level consumer group status on a topic

    e,g.
    %s status -z prod -c trade -t OrderStatusMsg

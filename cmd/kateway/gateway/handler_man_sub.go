...
// @rest GET /v1/fetch/:appid/:topic/:ver/:partition?sub=1&offset=xx&n=1&wait=5s
func (this *manServer) fetchHandler(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	isSub, _ = strconv.Atoi(q.Get("sub"))
	...
	if isSub == 1 {
		...
		if err := manager.Default.AuthSub(myAppid, r.Header.Get(HttpHeaderSubkey),
			...
			log.Error("fetch[%s] %s {topic:%s, ver:%s, hisapp:%s, myAppid:%s, subKey:%s}: %s",
				myAppid, r.RemoteAddr, topic, ver, hisAppid, myAppid, r.Header.Get(HttpHeaderSubkey), err)
		...
		// pub fetch
		if err := manager.Default.OwnTopic(myAppid, r.Header.Get(HttpHeaderPubkey), topic); err != nil {
			log.Error("fetch[%s] %s {topic:%s, ver:%s, hisapp:%s, myAppid:%s, pubKey:%s}: %s",
				myAppid, r.RemoteAddr, topic, ver, hisAppid, myAppid, r.Header.Get(HttpHeaderPubkey), err)
	...
	log.Info("fetch[%s] %s(%s) {app:%s topic:%s ver:%s partition:%d sub:%d offset:%d n:%d wait:%s}",
		myAppid, r.RemoteAddr, getHttpRemoteIp(r), hisAppid, topic, ver, partition, isSub, offset, followedN, waitParam)

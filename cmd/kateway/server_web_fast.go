	...
	if this.httpServer != nil {
		go func() {
			...
			for {
				...
				this.httpListener, err = net.Listen("tcp", this.webServer.Addr)
				if err != nil {
					if retryDelay == 0 {
				...
				log.Error(this.webServer.Serve(this.httpListener))
			}
		}()
		...
		this.gw.wg.Add(1)
		log.Info("%s http server ready on %s", this.name, this.webServer.Addr)
	}
